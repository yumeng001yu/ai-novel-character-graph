...
| POST | `/api/characters/search` | 角色搜索（POST，支持更复杂查询） |
...
| GET | `/api/graph/nodes` | 图谱节点查询 |
| GET | `/api/graph/edges` | 图谱边查询 |
| POST | `/api/graph/nodes` | 图谱节点批量查询 |
| POST | `/api/graph/edges` | 图谱边批量查询 |
| GET | `/api/knowledge-base/:novelId/question` | 知识库问答 |
| POST | `/api/knowledge-base/:novelId/question` | 知识库问答（POST） |
| GET | `/api/prompt-presets` | 列出所有提示词预设 |
| GET | `/api/prompt-presets/:id` | 获取单个预设 |
| POST | `/api/prompt-presets` | 创建预设 |
| PUT | `/api/prompt-presets/:id` | 更新预设 |
| DELETE | `/api/prompt-presets/:id` | 删除预设 |
| POST | `/api/prompt-presets/:id/set-default` | 设为默认预设 |
| GET | `/api/prompt-presets/macros/list` | 宏变量列表 |
...
│   │   │   ├── graph.route.ts
│   │   │   ├── character.route.ts
│   │   │   ├── character-chat.route.ts  # 角色对话路由
│   │   │   ├── snapshot.route.ts
│   │   │   ├── task.route.ts
│   │   │   ├── continue.route.ts
│   │   │   ├── settings.route.ts
│   │   │   ├── export.route.ts
│   │   │   ├── graphrag.route.ts        # 知识库问答路由
│   │   │   ├── knowledge-base.route.ts  # 知识库路由
│   │   │   └── prompt-preset.route.ts   # 提示词预设路由
│   │   ├── services/                    # 业务逻辑层
...
│   │   │   ├── ai-client.service.ts     # AI 调用（含 startBuffer 流式机制）
│   │   │   ├── character-chat.service.ts # 角色对话（含宏变量替换）
│   │   │   ├── graphrag.service.ts      # GraphRAG 知识库问答
│   │   │   ├── embedding.service.ts     # 向量嵌入
│   │   │   ├── vector-search.service.ts # 向量搜索
│   │   │   ├── reranker.service.ts      # 重排序
│   │   │   ├── settings.service.ts
...
│   │   │   │   ├── event.repo.ts
│   │   │   │   ├── relation.repo.ts
│   │   │   │   ├── text-chunk.repo.ts   # 文本块（向量搜索用）
│   │   │   │   └── text-segment.repo.ts
│   │   │   ├── redis/
│   │   │   │   ├── connection.ts        # Redis 连接（AOF 持久化）
│   │   │   │   ├── task-queue.repo.ts
│   │   │   │   ├── progress.repo.ts
...
│   │   │   └── character-disambiguate.prompt.ts
│   │   ├── utils/
│   │   │   ├── text-splitter.ts
│   │   │   ├── fingerprint.ts
│   │   │   ├── crypto.ts
...
│   │   ├── reranker-settings.json
│   │   ├── encryption.key
│   │   └── prompt-presets/              # 提示词预设 JSON 文件
│   ├── scripts/
│   │   └── benchmark.ts                 # 自动化测试基准
│   ├── tsconfig.json
│   └── package.json
...
│   │   │   │   ├── GraphTab.tsx         # 图谱标签页
│   │   │   │   ├── OriginalTextTab.tsx  # 原文标签页
│   │   │   │   └── QATab.tsx           # 问答标签页
│   │   │   ├── Knowledge/              # 知识库页：问答 + 侧边栏
│   │   │   ├── Settings/               # 设置页
│   │   │   │   ├── index.tsx           # AI 配置 + 构建配置
│   │   │   │   └── PromptPresetSettings.tsx  # 提示词预设设置
│   │   │   ├── Task/                   # 任务页
│   │   │   ├── Continue/               # 续建页
│   │   │   ├── Character/              # 角色页
│   │   │   └── Graph/                  # 图谱页
