...
export async function graphragRoutes(app: FastifyInstance) {
  // 全局 GraphRAG 查询（跨所有小说，支持 SSE 流式响应）
  app.post('/query', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (stream) {
      ...
      try {
        ...
        const result = await graphragService.globalQuery(question, onStream);
        reply.raw.write(`data: ${JSON.stringify({ type: 'done', sources: result.sources })}\n\n`);
        reply.raw.end();
      } catch (err: any) {
        logger.error(err, '全局 GraphRAG 查询失败');
        reply.raw.write(`data: ${JSON.stringify({ type: 'error', error: err.message })}\n\n`);
        reply.raw.end();
      }
    } else {
      try {
        const result = await graphragService.globalQuery(question);
        reply.send(result);
      } catch (err: any) {
        logger.error(err, '全局 GraphRAG 查询失败');
